...
2. **Handle Skip-generated callbacks (in progress October 20, 2025):** Added `@Sendable` annotations on bridge emitters and centralized bridge-error normalization inside `SkipZitiClient` so Swift mirrors skip-stripe’s pattern (no manual overrides; the shared client converts any propagated `Throwable` into `SkipZitiReportedError` events before rethrowing). Remaining work is Android/Kotlin-facing: (a) inject Kotlin-side helpers that wrap Skip’s auto-generated `callback_*` lambdas with overrides so `Throwable?` becomes `SkipZitiReportedError` before reaching Swift, eliminating the current `override` compile errors; (b) update this document with the conversion recipe once the helpers land, including references to the skip-stripe parity codepaths we mirrored today.
